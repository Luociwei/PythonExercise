...
from mix.driver.smartgiant.common.ic.tps6598x.tps6598x import TPS6598x as TPS6598xEmulator
from mix.driver.smartgiant.common.bus.i2c_bus_emulator import I2CEmulator
...
class PhoenixDef:
    ...
    PTN5110_EMULATOR_DEV = 'i2c_emulator'
...
class Phoenix(object):
    ...
    Args:
        tps6598x:   instance(TPS6598x)/None, class of TPS6598x which created outside. Default will create an emulator.
        ptn5110_i2c_bus:    instance(I2C)/None, is used to communicate with PTN5110. Default will create an emulator.
    ...
    Examples:
        ...
        vdm = Phoenix(tps6598x, ptn5110_i2c_bus, source_capabilities, emarker=False)

    '''

    # launcher will use this to match driver compatible string and load driver if matched.
    compatible = ["GQQ-VDM001002-000"]

    rpc_public_api = ['module_init', 'read_all_registers', 'read_register_by_address', 'read_register_by_name',
                      'flash_programmer', 'set_sink_capabilities', 'set_source_capabilities', 'read_all_adc',
                      'read_adc_by_name', 'set_firmware_region_pointer', 'reset', 'set_power_switch_config',
                      'set_pin_dir', 'set_pin', 'get_pin', 'read_firmware_version', 'change_source_pdo_count',
                      'change_pullup', 'write_register_by_address', 'write_register_by_name', 'enable_emarker',
                      'disable_emarker']

    def __init__(self, tps6598x=None, ptn5110_i2c_bus=None, source_capabilities=[], emarker=False):
        self.tps6598x = tps6598x or TPS6598xEmulator(PhoenixDef.TPS6598X_EMULATOR_DEVICE_ADDR)
        self.ptn5110_i2c_bus = ptn5110_i2c_bus or I2CEmulator(PhoenixDef.PTN5110_EMULATOR_DEV)
        self.source_capabilities = source_capabilities
        self.emarker = emarker

    def module_init(self):
        '''
        Configure ptn5110 to open mode, and config source capabilities.
        ...
        '''
        # detect ptn5110 ic
        read_data = self.ptn5110_i2c_bus.write_and_read(PhoenixDef.PTN5110_DEV_ADDR,
                                                        [PhoenixDef.PTN5110_ID_REGISTER], PhoenixDef.PTN5110_ID_LENGTH)
        if read_data == PhoenixDef.PTN5110_ID or \
                isinstance(self.ptn5110_i2c_bus, I2CEmulator):
            self.ptn5110_i2c_bus.write(PhoenixDef.PTN5110_DEV_ADDR,
                                       [PhoenixDef.PTN5110_ROLE_CONTROL_REGISTER,
                                        PhoenixDef.PTN5110_OPEN_MODE_CMD])
        else:
            raise PhoenixException("detect ptn5110 chip fail!")
        ...
        if isinstance(self.source_capabilities, list) and len(self.source_capabilities) > 0:
            try:
                ...
                assert isinstance(self.source_capabilities[0], dict) and self.source_capabilities[0]['voltage'] == 5000
            ...
            except (AssertionError, PhoenixException):
    ...
    def _int_2_byte_array(self, value):
        ...
        for i in range(PhoenixDef.INT_2_BYTE_ARRAY_LEN - len(byte_array)):
            byte_array.append(PhoenixDef.INT_2_BYTE_ARRAY_FILLING_VAL)
    ...
    def _FLrd(self, handle, address):
        ...
        if (cmdret == "Success"):
            (count, data_out) = device_rw.read_reg(
                handle, PhoenixDef.FLRD_WRITE_ADDR, PhoenixDef.FLRD_CHECK_DATA_LEN)
            return data_out
    ...
    def _GAID(self):
        ...
        '''
        device_rw.write_reg_4cc(self.tps6598x, PhoenixDef.GAID_WRITE_4CC_REG, 'GAID')
        device_rw.hw_sleep_ms(PhoenixDef.AGID_HW_SLEEP_MS)
    ...
    def _read_str_from(self, address):
        ...
        '''
        data_list = []
        for i in range(PhoenixDef.READ_STR_LOOP_TIMES):
            read_datas = self._FLrd(self.tps6598x, address)
            address += PhoenixDef.FLASH_WRITE_PAGE_SIZE
            data_list += list(read_datas)
            if data_list[-1] == PhoenixDef.STR_EOF_MARK:
                break

        data_length = data_list.index(PhoenixDef.STR_EOF_MARK)
    ...
    def _is_file_valid(self, file_name):
        ...
        with open(file_name, 'rb') as fd:
            read_data = fd.read(len(PhoenixDef.DEVICE_ID_LIST))
            read_id = array('B', read_data).tolist()
            if read_id != PhoenixDef.DEVICE_ID_LIST:
                raise PhoenixException('not a valid low-region file_name, %s != %s' %
                                       (read_id, PhoenixDef.DEVICE_ID_LIST))

    def _flash_erase(self, region):
        if 'Success' != hi_functions.FLem(self.tps6598x, region.APP_START_ADDR, PhoenixDef.APP_SECTOR_SIZE):
            raise PhoenixException("flash erase fail.")
        print("Falsh erase")

    def _flash_write(self, region, file_name):
        with open(file_name, 'rb') as fd:
            file_size = os.path.getsize(file_name)
            file_data = fd.read(PhoenixDef.FLASH_WRITE_PAGE_SIZE)
            write_addr = region.APP_START_ADDR

            while file_data:
                if 'Success' != hi_functions.FLad(self.tps6598x, write_addr):
                    raise PhoenixException("FLad function gets wrong when doing flash writing")
                if 'Success' != hi_functions.FLwd(self.tps6598x, array('B', file_data)):
                    raise PhoenixException("FLwd function gets wrong when doing flash writing")

                write_addr += len(file_data)
                file_data = fd.read(PhoenixDef.FLASH_WRITE_PAGE_SIZE)

                # Progress information printing
                if 0 == ((write_addr - region.APP_START_ADDR) % PhoenixDef.FIRMWARE_DIV_SIZE):
                    print('Flash written: %d %%' % (PhoenixDef.FLOAT_TO_PERCENT *
                                                    (write_addr - region.APP_START_ADDR) / file_size))

            # updata header
            if 'Success' != hi_functions.FLem(self.tps6598x, region.HEADER_ADDR, 1):
                raise PhoenixException("FLem function checks header wrong")

            if 'Success' != hi_functions.FLad(self.tps6598x, region.HEADER_ADDR):
                raise PhoenixException("FLad function checks header wrong")
            if 'Success' != hi_functions.FLwd(self.tps6598x, self._int_2_byte_array(region.APP_START_ADDR)):
                raise PhoenixException("FLwd function checks header wrong")
            ...
            if 'Success' != hi_functions.FLad(self.tps6598x, region.HEADER_ADDR + PhoenixDef.HEADER_OFFSET):
                raise PhoenixException("FLad sets offset fail")
            if 'Success' != hi_functions.FLwd(self.tps6598x, PhoenixDef.FLWD_OFFSET_CONFIG_ARRAY):
                raise PhoenixException("FLwd sets offset fail")
    ...
    def _flash_verify(self, region):
        ...
        if 0 != hi_functions.FLvy(self.tps6598x, app_addr):
            raise PhoenixException("Flash verify fail")
    ...
    def read_all_registers(self):
        '''
        Phoenix board read TPS6598x chip all registers informations.
    ...
    def read_register_by_address(self, register_address):
        '''
        Phoenix board read TPS6598x chip register information by address.
    ...
    def read_register_by_name(self, register_name):
        '''
        Phoenix board read TPS6598x chip register information by name.
    ...
    def write_register_by_address(self, register_address, field_name, value):
        '''
        Phoenix board write TPS6598x chip register information by address.
        ...
        Raise:
            PhoenixException: if register not support filed name will throw exception and raise what support
        ...
        if field_name not in field_list:
            raise PhoenixException('field name \'{}\' not in {}'.format(field_name, field_list))
    ...
    def write_register_by_name(self, register_name, field_name, value):
        '''
        Phoenix board write TPS6598x chip register information by name.
    ...
    def flash_programmer(self, region_name, firmware_name):
        '''
        Phoenix board burning TPS6598x chip.
        ...
        assert region_name in [PhoenixDef.REGION0_NAME, PhoenixDef.REGION1_NAME]

        if region_name == PhoenixDef.REGION0_NAME:
            region = REGION0
        else:
            region = REGION1
    ...
    def set_sink_capabilities(self, PDO_number, operating_current, voltage, min_current, max_current):
        '''
        Phoenix board config TPS6598x chip sink capabilities register.
        ...
        assert PDO_number >= PhoenixDef.PD0_MIN_NUMBER
        assert PDO_number <= PhoenixDef.PD0_MAX_NUMBER
        assert operating_current >= PhoenixDef.MIN_CURRENT
        assert operating_current <= PhoenixDef.MAX_CURRENT
        assert voltage >= PhoenixDef.MIN_VOLTAGE
        assert voltage <= PhoenixDef.MAX_VOLTAGE
        assert min_current >= PhoenixDef.MIN_CURRENT
        assert min_current <= PhoenixDef.MAX_CURRENT
        assert max_current >= PhoenixDef.MIN_CURRENT
        assert max_current <= PhoenixDef.MAX_CURRENT
    ...
    def set_source_capabilities(self, PDO_number, source_switch, voltage, max_current, peak_current='None'):
        '''
        Phoenix board config TPS6598x chip source capabilities register.
        ...
        Raise:
            PhoenixException, source switch not found.
        ...
        assert max_current >= PhoenixDef.MIN_CURRENT
        assert max_current <= PhoenixDef.MAX_CURRENT
        assert peak_current == 'None' or peak_current in PhoenixDef.PEAK_CURRENT_MAP

        if PDO_number == 1:
            if source_switch not in PhoenixDef.PDO1_SWITCH:
                raise PhoenixException("source switch not found in PD01")
        else:
            if source_switch not in PhoenixDef.PDOn_SWITCH:
                raise PhoenixException("source switch not found in PD0n")
        ...
        if PDO_number == 1:
            source_switch_value = PhoenixDef.PDO1_SWITCH[source_switch]
        else:
            source_switch_value = PhoenixDef.PDOn_SWITCH[source_switch]
        ...
        if not peak_current == 'None':
            register_definitions.TX_SOURCE_CAP.fieldByName(source_max_percentage_field_name).value = \
                PhoenixDef.PEAK_CURRENT_MAP[peak_current]
        register_definitions.TX_SOURCE_CAP.write(self.tps6598x)

    def read_all_adc(self):
        '''
        Phoenix board read TPS6598x chip all ADC value
    ...
    def read_adc_by_name(self, channel_name):
        '''
        Phoenix board read TPS6598x ADC informations by channel name
    ...
    def set_firmware_region_pointer(self, region_name):
        '''
        Phoenix board TPS6598x chip reboot with firmware in region0 or region1.
        ...
        Raise:
            PhoenixException, register write fail.
        ...
        if 'Success' != hi_functions.FLem(self.tps6598x, header_addr, 1):
            raise PhoenixException("change write start address fail")

        # write new pointer twice
        if 'Success' != hi_functions.FLad(self.tps6598x, header_addr):
            raise PhoenixException("FLad function write new pointer fail")
        if 'Success' != hi_functions.FLwd(self.tps6598x, self._int_2_byte_array(new_pointer)):
            raise PhoenixException("FLwd function write new pointer fail")
        if 'Success' != hi_functions.FLad(self.tps6598x, header_addr):
            raise PhoenixException("FLad function write new pointer fail")
        if 'Success' != hi_functions.FLwd(self.tps6598x, self._int_2_byte_array(new_pointer)):
            raise PhoenixException("FLwd function write new pointer fail")
        ...
        # set app offset with 0, in address=hearder_addr + 0x1000 - 4
        if 'Success' != hi_functions.FLad(self.tps6598x, header_addr + PhoenixDef.HEADER_OFFSET):
            raise PhoenixException("FLad function set app offset fail")
        if 'Success' != hi_functions.FLwd(self.tps6598x, PhoenixDef.FLWD_OFFSET_CONFIG_ARRAY):
            raise PhoenixException("FLwd function set app offset fail")
    ...
    def reset(self):
        '''
        Phoenix board cold restart TPS6598x function.
    ...
    def set_power_switch_config(self, power_port, state):
        '''
        Phoenix configurate TPS6598x power switch state as output, input or output_input etc.
    ...
    def set_pin_dir(self, gpio_number, state):
        '''
        Phoenix configurate TPS6598x gpio direction state to input or output.
        ...
        assert isinstance(state, basestring)
        assert gpio_number >= PhoenixDef.GPIO_MIN_NUM
        assert gpio_number <= PhoenixDef.GPIO_MAX_NUM
        assert state in [PhoenixDef.INPUT_DIR, PhoenixDef.OUTPUT_DIR]

        if state == PhoenixDef.INPUT_DIR:
    ...
    def set_pin(self, gpio_number, level):
        '''
        Phoenix set TPS6598x output gpio output level to high or low.
        ...
        assert gpio_number >= PhoenixDef.GPIO_MIN_NUM
        assert gpio_number <= PhoenixDef.GPIO_MAX_NUM
        assert level in [PhoenixDef.LOW_LEVEL, PhoenixDef.HIGH_LEVEL]

        if level == PhoenixDef.HIGH_LEVEL:
    ...
    def get_pin(self, gpio_number):
        '''
        Phoenix get TPS6598x gpio pin level.
    ...
    def read_firmware_version(self):
        ...
        '''
        fw0_version_string = self._read_str_from(PhoenixDef.REGION0_VERSION_ADDRESS)
        fw1_version_string = self._read_str_from(PhoenixDef.REGION1_VERSION_ADDRESS)
        return (fw0_version_string, fw1_version_string)

    def change_source_pdo_count(self, count):
        '''
        Phoenix change source pdo count.
        ...
        '''
        assert count >= PhoenixDef.NUM_PDOs_MIN and count <= PhoenixDef.NUM_PDOs_MAX

        register_definitions.TX_SOURCE_CAP.read(self.tps6598x)
        register_definitions.TX_SOURCE_CAP.fieldByName(PhoenixDef.NUM_PDOs_file_name).value = count
        register_definitions.TX_SOURCE_CAP.write(self.tps6598x)

    def change_pullup(self, pullup_mode):
        '''
        Phoenix change pullup.
