...
class VDM002001Exception(Exception):
...
class VDM002001(SGModuleDriver):
    '''
    TPS6598x USB Type-C & USB PD Controller Power Switch  function class

    compatible = ["GQQ-Q1QP-5-010"]

    Args:
        pd_i2c_1:    instance(I2C), class of I2C, which is used to control PTN5110.
        pd_i2c_2:    instance(I2C), class of I2C, which is used to control TPS6598x and CAT24C32.
    ...
    Examples:
        ...
        vdm = VDM002001(pd_i2c_1, pd_i2c_2, source_capabilities, emarker=False)
    ...
    def post_power_on_init(self, timeout=VDM002001Def.DEFAULT_TIMEOUT):
        '''
        ...
        while True:
            try:
                read_data = self.pd_i2c_1.write_and_read(VDM002001Def.PTN5110_DEV_ADDR,
                                                         [VDM002001Def.PTN5110_ID_REGISTER],
                                                         VDM002001Def.PTN5110_ID_LENGTH)
                if read_data == VDM002001Def.PTN5110_ID:
                    self.pd_i2c_1.write(VDM002001Def.PTN5110_DEV_ADDR,
                                        [VDM002001Def.PTN5110_ROLE_CONTROL_REGISTER,
                                         VDM002001Def.PTN5110_OPEN_MODE_CMD])
                else:
                    raise VDM002001Exception("detect ptn5110 chip fail!")

                self.reset(timeout)
                return
            except Exception as e:
                if time.time() - start_time > timeout:
                    raise VDM002001Exception("Timeout: {}".format(e.message))

    def reset(self, timeout=VDM002001Def.DEFAULT_TIMEOUT):
        '''
        ...
        while True:
            ...
            except (AssertionError, VDM002001Exception):
                raise VDM002001Exception("\r\n\
    ...
    def _int_2_byte_array(self, value):
        ...
        for i in range(VDM002001Def.INT_2_BYTE_ARRAY_LEN - len(byte_array)):
            byte_array.append(VDM002001Def.INT_2_BYTE_ARRAY_FILLING_VAL)
    ...
    def _FLrd(self, handle, address):
        ...
        for i in range(VDM002001Def.FLRD_WRITE_DATA_LEN - len(data_in)):
            data_in.append(0)
        device_rw.write_reg(handle, VDM002001Def.FLRD_WRITE_ADDR, data_in)
        device_rw.write_reg_4cc(handle, VDM002001Def.FLRD_WRITE_4CC_ADDR, 'FLrd')

        cmdret = hi_functions.verify_cmd_completed(handle, hi_functions.FLASH_HI_TIMEOUT)
        if (cmdret == "Success"):
            (count, data_out) = device_rw.read_reg(
                handle, VDM002001Def.FLRD_WRITE_ADDR, VDM002001Def.FLRD_CHECK_DATA_LEN)
            return data_out
    ...
    def _GAID(self):
        ...
        '''
        device_rw.write_reg_4cc(self.tps6598x, VDM002001Def.GAID_WRITE_4CC_REG, 'GAID')
        device_rw.hw_sleep_ms(VDM002001Def.AGID_HW_SLEEP_MS)
    ...
    def _read_str_from(self, address):
        ...
        '''
        data_list = []
        for i in range(VDM002001Def.READ_STR_LOOP_TIMES):
            read_datas = self._FLrd(self.tps6598x, address)
            address += VDM002001Def.FLASH_WRITE_PAGE_SIZE
            data_list += list(read_datas)
            if data_list[-1] == VDM002001Def.STR_EOF_MARK:
                break

        data_length = data_list.index(VDM002001Def.STR_EOF_MARK)
    ...
    def _is_file_valid(self, file_name):
        ...
        with open(file_name, 'rb') as fd:
            read_data = fd.read(len(VDM002001Def.DEVICE_ID_LIST))
            read_id = array('B', read_data).tolist()
            if read_id != VDM002001Def.DEVICE_ID_LIST:
                raise VDM002001Exception('not a valid low-region file_name, %s != %s' %
                                         (read_id, VDM002001Def.DEVICE_ID_LIST))

    def _flash_erase(self, region):
        if 'Success' != hi_functions.FLem(self.tps6598x, region.APP_START_ADDR, VDM002001Def.APP_SECTOR_SIZE):
            raise VDM002001Exception("flash erase fail.")
        print("Falsh erase")

    def _flash_write(self, region, file_name):
        with open(file_name, 'rb') as fd:
            file_size = os.path.getsize(file_name)
            file_data = fd.read(VDM002001Def.FLASH_WRITE_PAGE_SIZE)
            write_addr = region.APP_START_ADDR

            while file_data:
                if 'Success' != hi_functions.FLad(self.tps6598x, write_addr):
                    raise VDM002001Exception("FLad function gets wrong when doing flash writing")
                if 'Success' != hi_functions.FLwd(self.tps6598x, array('B', file_data)):
                    raise VDM002001Exception("FLwd function gets wrong when doing flash writing")

                write_addr += len(file_data)
                file_data = fd.read(VDM002001Def.FLASH_WRITE_PAGE_SIZE)

                # Progress information printing
                if 0 == ((write_addr - region.APP_START_ADDR) % VDM002001Def.FIRMWARE_DIV_SIZE):
                    print('Flash written: %d %%' % (VDM002001Def.FLOAT_TO_PERCENT *
                                                    (write_addr - region.APP_START_ADDR) / file_size))

            # updata header
            if 'Success' != hi_functions.FLem(self.tps6598x, region.HEADER_ADDR, 1):
                raise VDM002001Exception("FLem function checks header wrong")

            if 'Success' != hi_functions.FLad(self.tps6598x, region.HEADER_ADDR):
                raise VDM002001Exception("FLad function checks header wrong")
            if 'Success' != hi_functions.FLwd(self.tps6598x, self._int_2_byte_array(region.APP_START_ADDR)):
                raise VDM002001Exception("FLwd function checks header wrong")
            ...
            if 'Success' != hi_functions.FLad(self.tps6598x, region.HEADER_ADDR + VDM002001Def.HEADER_OFFSET):
                raise VDM002001Exception("FLad sets offset fail")
            if 'Success' != hi_functions.FLwd(self.tps6598x, VDM002001Def.FLWD_OFFSET_CONFIG_ARRAY):
                raise VDM002001Exception("FLwd sets offset fail")
    ...
    def _flash_verify(self, region):
        ...
        if 0 != hi_functions.FLvy(self.tps6598x, app_addr):
            raise VDM002001Exception("Flash verify fail")
    ...
    def read_all_registers(self):
        '''
        VDM002001 board read TPS6598x chip all registers informations.
    ...
    def read_register_by_address(self, register_address):
        '''
        VDM002001 board read TPS6598x chip register information by address.
    ...
    def read_register_by_name(self, register_name):
        '''
        VDM002001 board read TPS6598x chip register information by name.
    ...
    def write_register_by_address(self, register_address, field_name, value):
        '''
        VDM002001 board write TPS6598x chip register information by address.
        ...
        Raise:
            VDM002001Exception: if register not support filed name will throw exception and raise what support
        ...
        if field_name not in field_list:
            raise VDM002001Exception('field name \'{}\' not in {}'.format(field_name, field_list))
    ...
    def write_register_by_name(self, register_name, field_name, value):
        '''
        VDM002001 board write TPS6598x chip register information by name.
    ...
    def flash_programmer(self, region_name, firmware_name):
        '''
        VDM002001 board burning TPS6598x chip.
        ...
        assert region_name in [VDM002001Def.REGION0_NAME, VDM002001Def.REGION1_NAME]

        if region_name == VDM002001Def.REGION0_NAME:
            region = REGION0
        else:
            region = REGION1
    ...
    def set_sink_capabilities(self, PDO_number, operating_current, voltage, min_current, max_current):
        '''
        VDM002001 board config TPS6598x chip sink capabilities register.
        ...
        assert PDO_number >= VDM002001Def.PD0_MIN_NUMBER
        assert PDO_number <= VDM002001Def.PD0_MAX_NUMBER
        assert operating_current >= VDM002001Def.MIN_CURRENT
        assert operating_current <= VDM002001Def.MAX_CURRENT
        assert voltage >= VDM002001Def.MIN_VOLTAGE
        assert voltage <= VDM002001Def.MAX_VOLTAGE
        assert min_current >= VDM002001Def.MIN_CURRENT
        assert min_current <= VDM002001Def.MAX_CURRENT
        assert max_current >= VDM002001Def.MIN_CURRENT
        assert max_current <= VDM002001Def.MAX_CURRENT
    ...
    def set_source_capabilities(self, PDO_number, source_switch, voltage, max_current, peak_current='None'):
        '''
        VDM002001 board config TPS6598x chip source capabilities register.
        ...
        Raise:
            VDM002001Exception, source switch not found.
        ...
        assert max_current >= VDM002001Def.MIN_CURRENT
        assert max_current <= VDM002001Def.MAX_CURRENT
        assert peak_current == 'None' or peak_current in VDM002001Def.PEAK_CURRENT_MAP

        if PDO_number == 1:
            if source_switch not in VDM002001Def.PDO1_SWITCH:
                raise VDM002001Exception("source switch not found in PD01")
        else:
            if source_switch not in VDM002001Def.PDOn_SWITCH:
                raise VDM002001Exception("source switch not found in PD0n")
        ...
        if PDO_number == 1:
            source_switch_value = VDM002001Def.PDO1_SWITCH[source_switch]
        else:
            source_switch_value = VDM002001Def.PDOn_SWITCH[source_switch]
        ...
        if not peak_current == 'None':
            register_definitions.TX_SOURCE_CAP.fieldByName(source_max_percentage_field_name).value = \
                VDM002001Def.PEAK_CURRENT_MAP[peak_current]
        register_definitions.TX_SOURCE_CAP.write(self.tps6598x)

    def read_all_adc(self):
        '''
        VDM002001 board read TPS6598x chip all ADC value
    ...
    def read_adc_by_name(self, channel_name):
        '''
        VDM002001 board read TPS6598x ADC informations by channel name
    ...
    def set_firmware_region_pointer(self, region_name):
        '''
        VDM002001 board TPS6598x chip reboot with firmware in region0 or region1.
        ...
        Raise:
            VDM002001Exception, register write fail.
        ...
        if 'Success' != hi_functions.FLem(self.tps6598x, header_addr, 1):
            raise VDM002001Exception("change write start address fail")

        # write new pointer twice
        if 'Success' != hi_functions.FLad(self.tps6598x, header_addr):
            raise VDM002001Exception("FLad function write new pointer fail")
        if 'Success' != hi_functions.FLwd(self.tps6598x, self._int_2_byte_array(new_pointer)):
            raise VDM002001Exception("FLwd function write new pointer fail")
        if 'Success' != hi_functions.FLad(self.tps6598x, header_addr):
            raise VDM002001Exception("FLad function write new pointer fail")
        if 'Success' != hi_functions.FLwd(self.tps6598x, self._int_2_byte_array(new_pointer)):
            raise VDM002001Exception("FLwd function write new pointer fail")
        ...
        # set app offset with 0, in address=hearder_addr + 0x1000 - 4
        if 'Success' != hi_functions.FLad(self.tps6598x, header_addr + VDM002001Def.HEADER_OFFSET):
            raise VDM002001Exception("FLad function set app offset fail")
        if 'Success' != hi_functions.FLwd(self.tps6598x, VDM002001Def.FLWD_OFFSET_CONFIG_ARRAY):
            raise VDM002001Exception("FLwd function set app offset fail")
    ...
    def set_power_switch_config(self, power_port, state):
        '''
        VDM002001 configurate TPS6598x power switch state as output, input or output_input etc.
    ...
    def set_pin_dir(self, gpio_number, state):
        '''
        VDM002001 configurate TPS6598x gpio direction state to input or output.
        ...
        assert isinstance(state, basestring)
        assert gpio_number >= VDM002001Def.GPIO_MIN_NUM
        assert gpio_number <= VDM002001Def.GPIO_MAX_NUM
        assert state in [VDM002001Def.INPUT_DIR, VDM002001Def.OUTPUT_DIR]

        if state == VDM002001Def.INPUT_DIR:
    ...
    def set_pin(self, gpio_number, level):
        '''
        VDM002001 set TPS6598x output gpio output level to high or low.
        ...
        assert gpio_number >= VDM002001Def.GPIO_MIN_NUM
        assert gpio_number <= VDM002001Def.GPIO_MAX_NUM
        assert level in [VDM002001Def.LOW_LEVEL, VDM002001Def.HIGH_LEVEL]

        if level == VDM002001Def.HIGH_LEVEL:
    ...
    def get_pin(self, gpio_number):
        '''
        VDM002001 get TPS6598x gpio pin level.
    ...
    def read_firmware_version(self):
        '''
        VDM002001 read firmware version.
        ...
        '''
        fw0_version_string = self._read_str_from(VDM002001Def.REGION0_VERSION_ADDRESS)
        fw1_version_string = self._read_str_from(VDM002001Def.REGION1_VERSION_ADDRESS)
        return (fw0_version_string, fw1_version_string)

    def change_source_pdo_count(self, count):
        '''
        VDM002001 change source pdo count.
        ...
        '''
        assert count >= VDM002001Def.NUM_PDOs_MIN and count <= VDM002001Def.NUM_PDOs_MAX

        register_definitions.TX_SOURCE_CAP.read(self.tps6598x)
        register_definitions.TX_SOURCE_CAP.fieldByName(VDM002001Def.NUM_PDOs_file_name).value = count
        register_definitions.TX_SOURCE_CAP.write(self.tps6598x)

    def change_pullup(self, pullup_mode):
        '''
        VDM002001 change pullup.
