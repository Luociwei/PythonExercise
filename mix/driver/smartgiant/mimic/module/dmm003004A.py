...
from mix.driver.smartgiant.mimic.module.dmm003004 import DMM003004
...
class DMM003004A(DMM003004, Mimic):

    '''
    DMM003004A function class.

    compatible = ["GQQ-DMM003004A-000"]
    ...
    Examples:
        ...
        daqt1 = MIXDAQT1('/dev/MIX_DAQT1_0', ad717x_chip='AD7175' ad717x_mvref=vref, use_spi=False, use_gpio=False)
    ...
    def __init__(self, i2c=None, ipcore=None):
        if i2c:
            self.eeprom = CAT24C32(DMM003004ADef.EEPROM_DEV_ADDR, i2c)
            self.sensor = NCT75(DMM003004ADef.SENSOR_DEV_ADDR, i2c)
            self.cat9555 = CAT9555(DMM003004ADef.CAT9555_DEV_ADDR, i2c)
        else:
            self.eeprom = EepromEmulator('eeprom_emulator')
            self.sensor = NCT75Emulator('nct75_emulator')
            self.cat9555 = CAT9555Emulator(
                DMM003004ADef.CAT9555_DEV_ADDR, None, None)
        ...
        if ipcore:
            ...
            self.ad7175 = MIXAd7175SGEmulator(
                'mix_ad7175_sg_emulator', DMM003004ADef.EMULATOR_REG_SIZE)
        ...
        super(Mimic, self).__init__(self.eeprom, self.sensor,
                                    cal_table={}, range_table=dmm003004A_range_table)

        self.function_info = dmm003004A_function_info
        self.module_calibration_info = dmm003004A_range_table
