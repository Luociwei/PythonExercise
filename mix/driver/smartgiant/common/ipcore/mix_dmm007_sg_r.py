...
class MIXDMM007SGRException(Exception):
...
class MIXDMM007SGR(object):
    '''
    MIXDMM007SGR Driver
        MIXDMM007SGR aggregated IP includes MIXAD717x, MIXQSPI and MIXGPIO,
        MIXAD717x ipcore will be created, and change flags (use_spi, use_gpio)
        to decide which one to be created.

    ClassType = MIXDMM007SGR
    ...
    Examples:
        daqt1 = MIXDMM007SGR(axi4_bus='/dev/MIX_DAQT1_0', ad717x_chip='AD7175', ad717x_mvref=5000,
    ...
    def __init__(self, axi4_bus, ad717x_chip, ad717x_mvref=5000, code_polar="bipolar",
        ...
        if isinstance(axi4_bus, basestring):
            ...
            self.axi4_bus = AXI4LiteBus(axi4_bus, MIXDMM007SGRDef.REG_SIZE)
        else:
    ...
    def open(self, use_spi, use_gpio):
        ...
        self.ad717x_axi4_bus = AXI4LiteSubBus(self.axi4_bus, MIXDMM007SGRDef.MIX_AD717X_IPCORE_ADDR,
                                              MIXDMM007SGRDef.AD717X_REG_SIZE)
        ...
        if self.ad717x_chip == "AD7175":
            ...
            raise MIXDMM007SGRException("Unsupported AD717x chip type %s." % (self.ad717x_chip))

        # if use_gpio is True, MIXGPIOSG will be created.
        if use_gpio is True:
            self.gpio_axi4_bus = AXI4LiteSubBus(self.axi4_bus, MIXDMM007SGRDef.MIX_GPIO_IPCORE_ADDR,
                                                MIXDMM007SGRDef.GPIO_REG_SIZE)
            self.gpio = MIXGPIOSG(self.gpio_axi4_bus)
        # if use_spi is True, MIXQSPISG will be created.
        if use_spi is True:
            self.spi_axi4_bus = AXI4LiteSubBus(self.axi4_bus, MIXDMM007SGRDef.MIX_SPI_IPCORE_ADDR,
                                               MIXDMM007SGRDef.SPI_REG_SIZE)
            self.spi = MIXQSPISG(self.spi_axi4_bus)

    def close(self):
        '''
        MIXDMM007SGR close device
