...
from mix.driver.smartgiant.beast.module.scope001002_map import Scope001002
...
class Scope001002Debuger(cmd.Cmd):
    ...
    def do_post_power_on_init(self, line):
        ...
        self.scope001002.post_power_on_init(eval(line))
        print("Done.")

    @handle_errors
    def do_reset(self, line):
        '''reset timeout_s'''
        self.scope001002.reset(eval(line))
        print("Done.")

    @handle_errors
    def do_get_driver_version(self, line):
        '''get_driver_version'''
        self.scope001002.get_driver_version()
        print("Done.")
    ...
    def do_frequency_measure(self, line):
        '''frequency_measure
     measure input signal frequency function
    ...
    def do_vpp_measure(self, line):
        '''vpp_measure
     measure input signal vpp function
        duration: int, Measure millisecond time
        eg: vpp_measure 100 '''
        if '?' == line:
            print(get_function_doc(self))
    ...
    def do_rms_measure(self, line):
        '''rms_measure
     measure input signal rms value
        duration: int, Measure millisecond time
        eg: rms_measure 100 '''
        if '?' == line:
            print(get_function_doc(self))
...
if __name__ == '__main__':
    ...
    scope001002_dbg = create_scope001002_dbg(args.signal_meter, args.daqt2, args.i2c)

    scope001002_dbg.cmdloop()
