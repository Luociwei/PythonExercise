# -*- coding: utf-8 -*-
from mix.driver.smartgiant.blade.module.scope007001_map import BladeBase
...
class Scope007002(BladeBase):
    ...
    Examples:
        ...
        scope007002 = Scope007002(i2c=i2c_bus,
                            adc=ad7608,
                            xadc=xadc,
        ...
        Example for Analog input measurement:
            # scope007002 Analog input measurement, measure DC voltage with option `AVG` or `RMS`.
            adc_value = scope007002.adc_voltage_measure('AVG', 'ch0', '5V', 20000, 5, 100)
            print(adc_value)
            # Terminal shows "[xxx.xxx, 'mV']"

        Example for Analog output:
            # scope007002 dac output voltage, 0-5500 mV
            scope007002.dac_output('ch0', 1000)

        Example for AC signal measurement:
            # Signal: Square,100000Hz,2.5 VPP, 50%, High 1.25V

            # scope007002 measure ac signal with option 'f': freq
            result = scope007002.ac_signal_measure('f', 2500, 1000)
            print(result)
            # Terminal shows "{'freq': (xxx.x, 'Hz')}"

            # scope007002 measure ac signal with option 'v': vpp
            result = scope007002.ac_signal_measure('v', 2500, 1000)
            print(result)
            # Terminal shows "{'vpp': (xxx, 'mV')}"

            # scope007002 measure ac signal with option 'fwd': freq, width, duty
            result = scope007002.ac_signal_measure('fwd', 2500, 1000)
            print(result)
            # Terminal shows "{'duty': (xxx, '%%'), 'width': (xxx, 'us'), 'freq': (xxx, 'Hz')}"

        Example for ASK signal Encode:
            data = [1, 2, 3, 4, 5]
            scope007002.ask_write_encode_data(data)

        Example for FSK signal Decode:
            # scope007002 measure FSK signal frequency and receive data.
            freq = scope007002.fsk_frequency_measure(2000)
            print(freq)
            # FSK Decode State
            state = scope007002.fsk_decode_state()
            print(state)
            data = scope007002.fsk_read_decode_data()
            print(data)
        ...
        Example for data upload:
            ...
            scope007002.adc_measure_upload_enable('10V', 2000)
            time.sleep(1)
            ...
            scope007002.adc_measure_upload_disable()
