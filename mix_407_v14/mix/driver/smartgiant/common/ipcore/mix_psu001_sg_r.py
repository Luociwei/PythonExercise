...
class MIXPSU001SGRException(Exception):
...
class MIXPSU001SGR(object):
    '''
    MIXPSU001SGR Driver
        MIXPSU001SGR aggregated IP includes MIXAD717x, MIXQSPI and MIXGPIO,
        MIXAD717x ipcore will be created, and change flags (use_spi, use_gpio)
        to decide which one to be created.

    ClassType = MIXPSU001SGR

    Args:
        axi4_bus:        instance(AXI4LiteBus)/string/None, axi4lite bus or dev path.
        ad717x_chip:     string, ['AD7175', 'AD7177'], ADC chip type.
        ad717x_mvref:    int, default 2500, ADC reference voltage.
        use_spi:         boolean, spi use flag.
        use_gpio:        boolean, gpio use flag.

    Examples:
        psu001 = MIXPSU001SGR(axi4_bus='/dev/MIX_PSU001_SG_R_0', ad717x_chip='AD7175', ad717x_mvref=2500,
                              use_spi=False, use_gpio=False)

    '''

    def __init__(self, axi4_bus, ad717x_chip, ad717x_mvref=2500, code_polar="bipolar",
                 reference="internal", buffer_flag="enable", clock="crystal",
                 use_spi=False, use_gpio=False):
        if isinstance(axi4_bus, basestring):
            # device path; create axi4lite instance
            self.axi4_bus = AXI4LiteBus(axi4_bus, MIXPSU001SGRDef.REG_SIZE)
        else:
    ...
    def open(self, use_spi, use_gpio):
        ...
        self.ad717x_axi4_bus = AXI4LiteSubBus(self.axi4_bus, MIXPSU001SGRDef.MIX_AD717X_IPCORE_ADDR,
        ...
        if self.ad717x_chip == "AD7175":
            ...
            raise MIXPSU001SGRException("Unsupported AD717x chip type %s." % (self.ad717x_chip))

        # if use_gpio is True, MIXGPIOSG will be created.
        if use_gpio is True:
            self.gpio_axi4_bus = AXI4LiteSubBus(self.axi4_bus, MIXPSU001SGRDef.MIX_GPIO_IPCORE_ADDR,
                                                MIXPSU001SGRDef.GPIO_REG_SIZE)
            self.gpio = MIXGPIOSG(self.gpio_axi4_bus)
        # if use_spi is True, MIXQSPISG will be created.
        if use_spi is True:
            self.spi_axi4_bus = AXI4LiteSubBus(self.axi4_bus, MIXPSU001SGRDef.MIX_SPI_IPCORE_ADDR,
                                               MIXPSU001SGRDef.SPI_REG_SIZE)
            self.spi = MIXQSPISG(self.spi_axi4_bus)

    def close(self):
        '''
        MIXPSU001SGR close device
